"""Parametric bootstrap for MBG/NBD parameter uncertainty (issue #125, #141).

The seam under test is the public `MBGNBD.parameter_uncertainty()` and the
`ParameterUncertainty` it returns — the same engine and result type #140 built
for `BGNBD`, reused here. The MBG/NBD-specific check is that its own generative
simulator, the one with a time-zero dropout, agrees with its likelihood: a base
simulated from known parameters — including customers who die immediately after
the first purchase — refit, recovers the behaviour it was drawn from.
"""
...
from clvkit import MBGNBD, CustomerBase
from clvkit._result import Result
from clvkit.clv.mbgnbd import _simulate_summary
...
def _simulated_base(
    ...
    *,
    ...
    """A timing-only base drawn from known MBG/NBD parameters.
...
def fitted() -> MBGNBD:
    return MBGNBD().fit(_simulated_base(300, seed=1))
...
def test_calling_it_before_fit_raises():
    with pytest.raises(RuntimeError, match="not fitted"):
        MBGNBD().parameter_uncertainty(n=10, seed=0)
...
def test_the_simulator_and_likelihood_agree():
    # Simulate a large base from the known truth — time-zero dropouts included —
    # refit, and recover the two quantities the parameters exist to express: the
    # population purchase rate r/alpha and the population dropout probability
    # a/(a+b). This pins the source-tree simulator to the likelihood, exactly as
    # the model's own recovery tests do — a and b individually are too weakly
    # identified to assert on, which is the whole motivation for this feature.
    params = MBGNBD().fit(_simulated_base(5000, seed=2)).params_
...
def test_the_interval_covers_the_truth_for_the_well_identified_parameters():
    ...
    # show as uncertain.
    model = MBGNBD().fit(_simulated_base(2000, seed=3))
...
def test_a_and_b_come_back_wider_than_r():
    ...
    # wider. This is the honest result the feature exists to surface, and under
    # the MBG/NBD it is starker than under the BG/NBD — a and b run to enormous
    # bootstrap tails, so a summed-width test would be swamped by their absolute
    # magnitude; the relative width is what carries the point cleanly.
    model = MBGNBD().fit(_simulated_base(1500, seed=6))
